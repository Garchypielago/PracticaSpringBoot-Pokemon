...
public class ProductInCartServiceImpl implements ProductInCartService {
    ...
    public Optional<ProductInCart> findByPokemon(Long pokemonId){
        return productInCartRepository.findProductInCartByPokemon_IdAndUser_Id(
                pokemonId,
                authService.getCurrentAppUserId()
        );
    }
    @Override
    ...
    public long getQuantityByPokemonId(Long pokemonId) {
        return findByPokemon(pokemonId)
                .map(ProductInCart::getProductNumber)
                .orElse(0L);
    }
